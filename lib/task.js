...
var getExportDeclarations = function(file) {
  var output = '';
  ...
  file.match(regexFilter).forEach(function (line) {
    ...
    output = output + '\t' + camelCase(propName) + ': ' + rightHandSide + ";\n";
  });
  return output;
}
...
exports.buildThemeVariables = function (opts) {
  ...
  configPaths.forEach(function (configPath) {
    ...
    exportDeclarations.push(getExportDeclarations(themeVariableList));
  ...
  var formattedExportDeclarations = ':export {\n' + exportDeclarations.join('\n') + '}\n';
  themeVariables.push(formattedExportDeclarations);
